...
results/EXPECTED.md          every figure this code produces, with its provenance
...
Licensed under the MIT License, in `LICENSE`. This covers the code and the data files.
The licence text is identical to the one in `erdos126`.

The write-up is not kept in this repository. `results/EXPECTED.md` records every figure
the code produces, which is what a reader needs to check the numbers against.
